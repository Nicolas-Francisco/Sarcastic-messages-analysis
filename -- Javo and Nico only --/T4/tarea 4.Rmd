...
Si ahora calculamos los valores de *lift* por la primera regla, tendremos que:
...
Teniendo que en realidad ambos son estadísticamente independientes entre sí, y en realidad el atributo `audífonos` no aporta realmente información (redundancia). Luego podemos concluir que la regla más correcta es `{smartphone} => {audífonos}`, y sería la pormoción más lógica para este negocio.